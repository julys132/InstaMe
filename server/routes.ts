...
import {
  deleteObject as deleteBucketObject,
  getObject as getBucketObject,
  isObjectBucketConfigured,
  uploadObject as uploadBucketObject,
...
const TEMPORARY_IMAGE_SERVICE_MESSAGE =
  "We're receiving an unusually high number of image requests right now. Please try again in about 30 minutes.";

function toUserFacingTemporaryImageServiceMessage(errorMessage: string): string | null {
  ...
  return providerCapacityPatterns.some((pattern) => pattern.test(normalized))
    ? TEMPORARY_IMAGE_SERVICE_MESSAGE
    : null;
}

function shouldFallbackTogetherToGemini(error: unknown): boolean {
...
function isGeminiNoImageDataError(error: unknown): boolean {
  const normalized = normalizeStringValue(toErrorMessage(error, "")).toLowerCase();
...
let appleNotificationRootCertCache: { cert: X509Certificate; fetchedAt: number } | null = null;
const APPLE_NOTIFICATION_ROOT_CERT_TTL_MS = 24 * 60 * 60 * 1000;
...
function getAppleNotificationRootCertUrl(): string {
  return (
    normalizeStringValue(process.env.APPLE_SERVER_NOTIFICATION_ROOT_CERT_URL) ||
    "https://www.apple.com/certificateauthority/AppleRootCA-G3.cer"
  );
}

async function getTrustedAppleNotificationRootCertificate(): Promise<X509Certificate> {
  if (
    appleNotificationRootCertCache &&
    Date.now() - appleNotificationRootCertCache.fetchedAt < APPLE_NOTIFICATION_ROOT_CERT_TTL_MS
  ) {
...
function assertCertificateIsValidNow(cert: X509Certificate, label: string) {
  const now = Date.now();
...
async function verifyAppleServerSignedPayload(signedPayload: string): Promise<Record<string, unknown>> {
...
function decodeAppleSignedSubPayload(input: unknown): Record<string, unknown> | null {
...
const DEFAULT_APPLE_SUBSCRIPTION_PRODUCT_TO_PLAN: Record<string, string> = {
...
function getSubscriptionPlanById(planId: string): SubscriptionPlan | undefined {
...
const MAX_INSTAME_SAVED_PACKS = 30;
const MAX_INSTAME_PACK_IMAGES = 16;
const MAX_INSTAME_PACK_IMAGE_BASE64_LENGTH = 20_000_000;
...
const APPLE_IAP_SUBSCRIPTION_PRODUCTS: Record<string, string> = {
...
function normalizeTransformIntensity(input: unknown): TransformIntensity {
...
function parseAppleMilliseconds(input: unknown): number {
...
function parseAppleBooleanFlag(input: unknown): boolean | null {
  const value = normalizeStringValue(input).toLowerCase();
...
function toInstaMePackThumbUri(
  image: { previewBase64?: string | null; inlineBase64?: string | null; mimeType?: string | null } | null | undefined,
): string | null {
...
async function resolveInstaMePackImageBytes(
  userId: string,
  packId: string,
  imageId: string,
  ...
  const [image] = await db
    .select()
    .from(instamePackImages)
    .where(
      and(
        eq(instamePackImages.id, imageId),
        eq(instamePackImages.packId, packId),
        eq(instamePackImages.userId, userId),
      ),
    );
  ...
  if (image.storageKey) {
    const object = await getBucketObject(image.storageKey);
    if (object) {
      buffer = object.body;
...
async function deleteInstaMePacks(userId: string, packIds: string[]): Promise<void> {
  if (packIds.length === 0) return;

  const images = await db
    .select({ storageKey: instamePackImages.storageKey })
    .from(instamePackImages)
    .where(and(eq(instamePackImages.userId, userId), inArray(instamePackImages.packId, packIds)));

  for (const image of images) {
    if (image.storageKey) {
      try {
        await deleteBucketObject(image.storageKey);
      } catch (error) {
  ...
  // pack_images rows cascade-delete when the parent pack row is removed.
  await db
    .delete(instamePacks)
    .where(and(eq(instamePacks.userId, userId), inArray(instamePacks.id, packIds)));
}
...
function buildOldMoneyTransformPrompt(options: {
  ...
  const backgroundRule = options.preserveBackground
...
async function generateGeminiImageFromParts(options: {
  ...
  const requestImage = async (requestModel: string): Promise<{ imageBase64: string; model: string; provider: string }> => {
...
async function analyzeOwnStyleReferenceImage(
  ...
  const analysisPrompt = isCreativePromptMode
    ? CREATIVE_OWN_STYLE_ANALYSIS_PROMPT
    : OWN_STYLE_ANALYSIS_PROMPT;
  const primaryModel = isCreativePromptMode
    ? DEFAULT_CREATIVE_OWN_STYLE_ANALYSIS_MODEL
    : DEFAULT_OWN_STYLE_ANALYSIS_MODEL;
  const fallbackModels = Array.from(
    new Set(
      [DEFAULT_OWN_STYLE_ANALYSIS_MODEL, DEFAULT_STYLE_TEXT_MODEL].filter(
        (model): model is string => Boolean(model) && model !== primaryModel,
      ),
    ),
  );
  ...
  const runAnalysis = async (model: string): Promise<string> => {
    ...
    const analysisText = isCreativePromptMode
      ? rawAnalysisText.trim()
      : rawAnalysisText
          .replace(/\r\n/g, "\n")
          .replace(/[ \t]+\n/g, "\n")
          .replace(/\n{3,}/g, "\n\n")
...
async function generateInstaMeEditImage(options: {
  ...
  try {
    ...
    const parts: GeminiPart[] = [
      ...
      {
        text:
          "Primary generated portrait to edit. Keep this person and composition consistent unless the user explicitly requested a change:",
      },
      ...toGeminiInlineImageParts([options.currentImage]),
      ...(options.originalPhoto
        ? [
            { text: "Original base portrait for identity anchoring:" } as GeminiPart,
...
function getAppleReceiptTransactions(appleResult: any): any[] {
...
async function getUserCreditsSubscriptionState(userId: string) {
  const [user] = await db
    .select({
...
async function linkAppleOriginalTransactionId(userId: string, originalTransactionId: string | null) {
...
async function applyAppleSubscriptionNotificationUpdate(params: {
  ...
  if (
    ...
    resolveSubscriptionProvider(currentUser) === "apple" &&
    (!normalizedOriginalTransactionId || currentUser.appleOriginalTransactionId === normalizedOriginalTransactionId)
  ) {
    await db
      .update(users)
      .set({
...
async function findUserForAppleNotification(params: {
  ...
  if (accountToken) {
    const [userById] = await db
      .select({ id: users.id })
      .from(users)
      .where(eq(users.id, accountToken));
    if (userById) {
      return userById;
    }
  }
  ...
  const [userByOriginalTransaction] = await db
    .select({ id: users.id })
    .from(users)
...
async function syncAppleSubscriptionReceiptState(params: {
  ...
  for (const transaction of receiptTransactions) {
    ...
    if (grantResult.status === "duplicate" && grantResult.existingOwnerId && grantResult.existingOwnerId !== params.userId) {
      return { status: "claimed_by_other_user" as const };
    }
  ...
  const activeTransaction = receiptTransactions
    .filter((transaction: any) => Number.parseInt(String(transaction?.cancellation_date_ms || "0"), 10) <= 0)
    .sort((a: any, b: any) => {
  ...
  if (activeTransaction) {
    ...
    const activePlan = activePlanId ? getSubscriptionPlanById(activePlanId) : undefined;
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/apple/server-notifications", async (req, res) => {
    try {
      ...
      const payload = (await verifyAppleServerSignedPayload(signedPayload)) as AppleServerNotificationPayload;
      const data = payload.data || {};
      const transactionPayload = decodeAppleSignedSubPayload(data.signedTransactionInfo) as AppleSignedTransactionPayload | null;
      const renewalPayload = decodeAppleSignedSubPayload(data.signedRenewalInfo) as AppleSignedRenewalPayload | null;
      ...
      if (autoRenewStatus === false && expiresMs <= Date.now()) {
        ...
        if (
          currentUser?.subscriptionPlan &&
          resolveSubscriptionProvider(currentUser) === "apple" &&
          (!originalTransactionId || currentUser.appleOriginalTransactionId === originalTransactionId)
        ) {
          await db
            .update(users)
            .set({
  ...
  // ---------------------------------------------------------------------------
  // Saved photo packs (composite preview + extracted individual images).
  // ---------------------------------------------------------------------------

  app.get("/api/instame/packs", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    const packs = await db
      .select()
      .from(instamePacks)
      .where(eq(instamePacks.userId, userId))
      .orderBy(desc(instamePacks.createdAt));
    ...
    const previewByPackId = new Map<string, (typeof images)[number]>();
    const imageCountByPackId = new Map<string, number>();
    for (const image of images) {
  ...
  app.get("/api/instame/packs/:packId", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    ...
    const [pack] = await db
      .select()
      .from(instamePacks)
      .where(and(eq(instamePacks.id, packId), eq(instamePacks.userId, userId)));
    ...
    const images = await db
      .select()
      .from(instamePackImages)
      .where(and(eq(instamePackImages.packId, packId), eq(instamePackImages.userId, userId)))
      .orderBy(instamePackImages.position);

    const preview = images.find((image) => image.role === "preview") || null;
    const individualImages = images
      .filter((image) => image.role !== "preview")
  ...
  app.get("/api/instame/packs/:packId/images/:imageId/raw", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    const packId = normalizeStringValue(req.params.packId);
  ...
  app.get("/api/instame/packs/:packId/images/:imageId", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    const packId = normalizeStringValue(req.params.packId);
    const imageId = normalizeStringValue(req.params.imageId);
  ...
  app.post("/api/instame/packs", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    const body = req.body || {};
    ...
    const previewInput =
      body.preview && typeof body.preview === "object" ? (body.preview as Record<string, unknown>) : null;
    const imagesInput = Array.isArray(body.images) ? body.images : [];

    type ParsedPackImage = {
      role: "preview" | "image";
      position: number;
      label: string;
      mimeType: string;
    ...
    const parsePackImage = (
      raw: unknown,
      role: "preview" | "image",
      fallbackPosition: number,
    ): ParsedPackImage | null => {
      if (!raw || typeof raw !== "object") return null;
      const record = raw as Record<string, unknown>;
      const base64 = stripDataUriPrefix(normalizeStringValue(record.base64)).replace(/\s+/g, "");
      if (!base64) return null;
      const previewBase64 =
        stripDataUriPrefix(normalizeStringValue(record.previewBase64)).replace(/\s+/g, "") || base64;
      const mimeType =
    ...
    const parsedImages: ParsedPackImage[] = [];
    const parsedPreview = parsePackImage(previewInput, "preview", 0);
    if (parsedPreview) {
      parsedImages.push(parsedPreview);
    }
    imagesInput.slice(0, MAX_INSTAME_PACK_IMAGES).forEach((raw: unknown, index: number) => {
    ...
    try {
      const imageRows: Array<typeof instamePackImages.$inferInsert> = [];
      for (let index = 0; index < parsedImages.length; index += 1) {
        const image = parsedImages[index]!;
        const imageId = randomUUID();
        let storageKey: string | null = null;
        let inlineBase64: string | null = image.base64;
      ...
      await db.delete(instamePacks).where(eq(instamePacks.id, created.id));
    ...
    // Rolling window: evict oldest packs beyond the cap (and their bucket objects).
    try {
      const allPacks = await db
        .select({ id: instamePacks.id })
        .from(instamePacks)
        .where(eq(instamePacks.userId, userId))
        .orderBy(desc(instamePacks.createdAt));
  ...
  app.delete("/api/instame/packs/:packId", authMiddleware, async (req, res) => {
    const userId = req.user!.id;
    const packId = normalizeStringValue(req.params.packId);
  ...
  app.post("/api/instame/grid-pipeline/extend", authMiddleware, async (req, res) => {
    ...
    const ctx: GridContinuityContext = {
    ...
    try {
      ...
      nextContinuityContext.usedScenes = [
        ...new Set([...(ctx.usedScenes || []), ...nextContinuityContext.usedScenes])
          .values(),
      ]
        .map((scene) => sanitizeGridPromptText(scene))
        .filter(Boolean);
      nextContinuityContext.usedHairstyles = [
        ...new Set([...(ctx.usedHairstyles || []), ...nextContinuityContext.usedHairstyles])
          .values(),
      ]
